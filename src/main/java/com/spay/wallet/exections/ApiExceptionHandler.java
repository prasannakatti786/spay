...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ApiExceptionResponse> apiExceptionResponse(ApiException e) {
        var apiException = new ApiExceptionResponse(e.getMessage(), LocalDateTime.now(),e.getStatus());
...
//    }
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatusCode status, WebRequest request) {
        ...
        var apiException = new ApiExceptionResponse(exception, LocalDateTime.now(),HttpStatus.valueOf(status.value()));
